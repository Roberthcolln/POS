...
$konf = FacadesDB::table('setting')->first();
$newPesanCount = Pemesanan::where('status', 'pending')->count();
$newBarCount = BarOrder::where('status', 'approved')->count();
$newKitchenCount = KitchenOrder::where('status', 'approved')->count();
$kasirCount = KasirTransaction::where('status', 'pending')->count();
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="hold-transition sidebar-mini">
  <div class="wrapper">
          ...
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item">
              <a href="{{ route('kasir.index') }}" class="nav-link">
                ...
                <p> Kasir</p>
                @if ($kasirCount > 0)
                <span class="right badge badge-danger">{{ $kasirCount }}</span>
                @endif
              </a>
            </li>
            ...
            <li class="nav-item">
              <a href="{{ route('pemesanan.index') }}" class="nav-link">
                ...
                <p> Pemesanan
                  @if ($newPesanCount > 0)
                  <span class="right badge badge-danger">{{ $newPesanCount }}</span>
                  @endif
                </p>
              </a>
            </li>
            <li class="nav-item">
              <a href="{{ route('bar.index') }}" class="nav-link">
                ...
                <p> BAR</p>
                @if ($newBarCount > 0)
                <span class="right badge badge-danger">{{ $newBarCount }}</span>
                @endif
              </a>
            </li>
            <li class="nav-item">
              <a href="{{ route('kitchen.index') }}" class="nav-link">
                <i class="fas fa-sink nav-icon"></i>
                <p> Kitchen</p>
                @if ($newKitchenCount > 0)
                <span class="right badge badge-danger">{{ $newKitchenCount }}</span>
                @endif
              </a>
            </li>
    ...
    <footer class="main-footer">
      ...
      <strong>Copyright &copy; <?php $cpy = date('Y');
                                echo $cpy; ?><a href="{{ route('dashboard.index') }}"> {{ $konf->instansi_setting }}</a>.</strong> All rights reserved.
    </footer>
  ...
  <script type="text/javascript">
    ...
    // Format the number input with thousand separator
        ...
        $.ajax({
          ...
          success: function(result) {
            ...
            $.each(result.kota, function(key, value) {
              $("#kota-dd").append('<option value="' + value
